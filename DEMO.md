...
| Uploads | Images only. Per file `DEMO_MAX_UPLOAD_MB` (12), at most `DEMO_MAX_UPLOADS` (100) live at once and `DEMO_MAX_TOTAL_UPLOAD_MB` (300) across all users. Each upload is deleted `DEMO_UPLOAD_TTL_SECONDS` (5) after it lands; the web grid refreshes itself when that happens. |
...
3. **Variables** (service → *Variables*):
   ...
   | `DEMO_UPLOAD_TTL_SECONDS` | optional, default `5`. Consider `60` so a visitor can open what they just uploaded. |
...
> read-only, uploads are removed automatically after a few seconds, and "Free up space"
